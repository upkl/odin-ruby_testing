...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'When the previous step is 18 and the random number is 8' do
      subject(:my_game) { described_class.new(8) }
      it 'returns 10' do
        previous_step = 18
        result = my_game.subtract_random_number(previous_step)
  ...
  describe '#play' do
    context 'when the random number is 0' do
      subject(:my_game) { described_class.new(0) }
    ...
    context 'when the random number is 47' do
      subject(:my_game) { described_class.new(47) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = my_game.play
        expect(result).to eq(7)
    ...
    context 'when the random number is -33' do
      subject(:my_game) { described_class.new(-33) }
      # remove the 'x' before running this test
      it 'will return 7' do
        result = my_game.play
        expect(result).to eq(7)
